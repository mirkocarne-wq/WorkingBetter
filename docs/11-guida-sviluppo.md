...
```
apps/
  api/        NestJS + Fastify. Moduli: auth (sessioni, password, inviti, SSO OIDC per tenant), core (persone, import CSV, org, ruoli), objectives, one-on-one, feedback, notifications (in-app, preferenze), forms (definizioni versionate, compilazioni, hook di invio), reviews (template, cicli, review, contesto), analytics (catalogo, query engine, alert, report di processo, export), surveys (template, inviti, risposte anonime, risultati con soglie), audit, health
  web/        Next.js (App Router). Login dev, dashboard, obiettivi (albero + check-in), 1:1, feedback e riconoscimenti, review (cicli HR, team, self-review, condivisione e firma), report (KPI, trend, segnali, processo, export), survey (compilazione, risultati, sintesi), form, notifiche, persone, utenti e accessi, impostazioni
