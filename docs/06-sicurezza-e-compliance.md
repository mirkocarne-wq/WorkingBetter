...
- **Rate limiting** sugli endpoint pubblici (login, password dimenticata e reset, accettazione invito, scambio codice SSO, login di sviluppo, feed iCalendar, questionari 360° e pre-boarding via magic link, callback OAuth dei connettori): finestra scorrevole per indirizzo IP, risposta `429` in formato Problem con `Retry-After`. Lo stato è in memoria per istanza: con più repliche il limite effettivo è moltiplicato per il numero di repliche, e va spostato su Redis o sul gateway quando si scala (nota in `docs/13`).
...
- **Connettori esterni** (ADR-0012): i client secret delle app OAuth e i token di accesso/refresh di Google, Microsoft e Slack sono cifrati con la chiave per tenant (AES-256-GCM, HKDF dalla master key) e non escono mai dall'API (le risposte espongono solo la presenza del segreto); lo *state* del flusso OAuth è firmato con scadenza di 10 minuti e lega tenant, utente e provider; i token si usano solo nel worker, che segna l'account «da ricollegare» al primo 401 senza ritentare; scollegare cancella i token. I messaggi Slack/Teams contengono solo titolo, anteprima e link, come le email (INT §4).
