...
- **Rate limiting** sugli endpoint pubblici (login, password dimenticata e reset, accettazione invito, scambio codice SSO, login di sviluppo, feed iCalendar, questionari 360° e pre-boarding via magic link): finestra scorrevole per indirizzo IP, risposta `429` in formato Problem con `Retry-After`. Lo stato è in memoria per istanza: con più repliche il limite effettivo è moltiplicato per il numero di repliche, e va spostato su Redis o sul gateway quando si scala (nota in `docs/13`).
