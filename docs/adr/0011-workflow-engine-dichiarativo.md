...
- Le app native con logica propria (survey, 360°) **non** vengono migrate sul motore generico: anonimato, soglie e report non devono entrare nel motore L2. La **review** converge (sprint 16) e l'**onboarding** anche (sprint 17: percorso = istanza silenziosa con una fase per task in un unico gruppo parallelo; il modulo resta padrone di ruoli, buddy, survey e notifiche e sincronizza completamenti, salti, riaperture, scadenze e assegnatari; le istanze «gestite dal modulo» non si decidono dal motore e rimandano alla pagina del modulo con `module_link`). Nel dettaglio della review: il ciclo diventa un'app «silenziosa» per ciclo (`reviewTemplateToApp`), ogni review un'istanza; il modulo REV mantiene rating, visibilità, notifiche e firma e ascolta la conclusione delle fasi con un hook del motore (`onStageDone`). I moduli nativi usano API interne senza controllo dei permessi del motore (`launchInternal`, `decideInternal`, `reopenTo`, `completeInternal`), perché i permessi li verificano loro.
- Azioni automatiche (APP-024, sprint 16): fase `action` con action item, attributo persona, webhook e avvio di un'altra app; eseguite in sequenza all'attivazione, esiti nel run e nel log, nessun blocco del processo in caso di errore. Dallo sprint 17 i webhook falliti vengono ritentati dal worker con backoff (coda `webhook_deliveries`, 5 tentativi).
- Limiti accettati: nessun ciclo arbitrario (solo rimando all'indietro), condizioni su un solo campo, nessuna firma dei webhook, nessun editor grafico (l'editor è una lista di fasi con anteprima).
