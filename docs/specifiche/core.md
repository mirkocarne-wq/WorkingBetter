...
- **Audit consultabile** (CORE-051, sprint 29): `GET /audit` (`audit:read`) con filtri per azione (prefisso), tipo di entità, entità, utente, periodo e testo, paginazione per data; ogni riga espone chi, cosa, quando, su quale entità, IP e **l'elenco dei campi cambiati** (`changedFields`, calcolato da `before`/`after`), **mai i valori**: i contenuti restano nel database e non passano dall'interfaccia, così l'audit è consultabile dall'HR admin senza diventare un canale di lettura di review e note. `GET /audit/actions` elenca le azioni presenti per il filtro; `GET /audit/export` produce il CSV con gli stessi filtri (massimo 10 000 righe) ed è a sua volta tracciato (`audit.export`). Web: Impostazioni → Audit.
- **Vista «cosa vede X»** (CORE-044, sprint 29): `GET /users/:id/access` (`roles:manage`) restituisce, per un utente, i ruoli assegnati (con nome, base e perimetro registrato), i ruoli impliciti, i **permessi effettivi raggruppati per modulo** con l'indicazione dei moduli spenti nel tenant, il perimetro di dato (riporti diretti e indiretti, unità del perimetro), il profilo della Guida e il livello di visibilità dei campi custom. Web: Persone → Utenti → «Cosa vede». **Assunzione**: il perimetro per unità delle assegnazioni HRBP (`scope_type = org_unit`) è registrato e mostrato ma non ancora applicato dai moduli, che oggi distinguono solo tenant e team del manager; la vista lo dice esplicitamente.
