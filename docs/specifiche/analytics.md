...
| ANA-006 | **Panoramica con andamento** (sprint 30): in Home, per chi ha un perimetro di analisi (HR: azienda; manager: team), fino a quattro indicatori dei moduli attivi con valore corrente, variazione rispetto all'inizio della finestra (14 giorni) e sparkline; ogni indicatore porta al Report con quell'andamento. Stessa fascia in testa al Report, con la finestra scelta (14/30/90 giorni). Endpoint `GET /analytics/overview?metrics=&days=` (perimetro e soglie del semantic layer; nessuna serie sotto soglia) | P0 |
| ANA-007 | **Grafici del design system** (sprint 30): un solo kit SVG (`apps/web/components/charts.tsx`) per sparkline, indicatore con variazione, barre orizzontali per categoria, distribuzione parte-sul-tutto, colonne per categorie ordinate e piccoli multipli. Regole in `docs/07` §Grafici: una scala per grafico, marcatori sottili, etichette in inchiostro, tooltip al passaggio, legenda per ≥ 2 serie, vista tabella dove conta la lettura puntuale, palette serie validata per daltonismo e contrasto. Usato in Home (salute obiettivi, segnali HR), Obiettivi (confidenza e progresso del periodo), Report (confronto per dimensione, altri andamenti), report di processo (completamento per unità/manager, distribuzione dei rating) e risultati survey (eNPS) | P0 |
