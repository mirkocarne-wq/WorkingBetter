...
// Ruolo custom dimostrativo (CORE-041): referente welfare senza gli altri poteri HR
await db.insert(roleDefinitions).values({ tenantId: T, key: 'welfare_admin', name: 'Referente welfare', description: 'Gestisce piani, catalogo, richieste e flussi paghe del welfare senza accedere a review e valutazioni.', baseRole: 'employee', permissions: ['people:read', 'org:read', 'notifications:read', 'welfare:use', 'welfare:manage', 'welfare:payroll', 'analytics:query'] });

// Automazioni dimostrative (APP-037/038): una a tempo e una su evento
await db.insert(automationRules).values([
  { tenantId: T, name: 'Colloquio di fine prova a 90 giorni', description: 'A 90 giorni dall’ingresso il manager riceve un’azione e la persona un promemoria.', enabled: true, trigger: { event: 'person.tenure', days: 90 }, conditions: [], actions: [{ type: 'action_item', title: 'Colloquio di fine periodo di prova', assignee: 'manager', dueDays: 7 }, { type: 'notify', to: ['subject'], message: 'Sono passati 90 giorni dal tuo ingresso: il tuo manager ti proporrà un colloquio di fine prova.' }] },
  { tenantId: T, name: 'Rating basso: coinvolgi HR', description: 'Quando una review chiude con rating 1 o 2, l’HRBP riceve un’azione per pianificare un piano di miglioramento.', enabled: true, trigger: { event: 'review.completed' }, conditions: [{ field: 'rating', op: 'lte', value: 2 }], actions: [{ type: 'action_item', title: 'Valuta un piano di miglioramento', assignee: 'hr', dueDays: 10 }, { type: 'notify', to: ['hr'], message: 'Una review si è chiusa con un rating basso: verifica se serve un piano di miglioramento.' }] },
  { tenantId: T, name: 'Formazione approvata: aggiorna la scheda', description: 'Quando una richiesta di formazione viene approvata, annota l’ultima formazione nel campo custom.', enabled: false, trigger: { event: 'app.completed', appKey: 'training_request' }, conditions: [{ field: 'outcome', op: 'eq', value: 'approved' }], actions: [{ type: 'person_field', field: 'custom:last_training', value: 'approvata' }] },
]);
