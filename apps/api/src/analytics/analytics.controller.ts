...
export class AnalyticsController {
  ...
  async query(@Query(new ZodValidationPipe(queryDto)) q: z.infer<typeof queryDto>, @Res({ passthrough: true }) reply: FastifyReply) {
    return q.format === 'csv' ? csv(reply, 'report.csv', await this.svc.queryCsv(q)) : this.svc.query(q);
  }

  @Get('trend') @RequirePermission(Q, T) @ApiOperation({ summary: 'Serie giornaliera di una metrica' }) trend(@Query(new ZodValidationPipe(trendDto)) q: z.infer<typeof trendDto>) { return this.svc.trend(q); }
  ...
  async alerts(@Query(new ZodValidationPipe(formatDto)) q: z.infer<typeof formatDto>, @Res({ passthrough: true }) reply: FastifyReply) {
  ...
  async process(@Param('cycleId', ParseUUIDPipe) id: string, @Query(new ZodValidationPipe(formatDto)) q: z.infer<typeof formatDto>, @Res({ passthrough: true }) reply: FastifyReply) {
