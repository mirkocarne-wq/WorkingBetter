...
import { countResponse } from '../common/responses.js';
...
export class NotificationsController {
  ...
  @Get('notifications') @RequirePermission(Permissions.NOTIFICATIONS_READ) list(@ZQuery(listQuery) q: z.infer<typeof listQuery>) { return this.svc.list(q); }
  @Get('notifications/unread-count') @RequirePermission(Permissions.NOTIFICATIONS_READ) @ZOk(countResponse) async unread() { return { count: await this.svc.unreadCount() }; }
  ...
  @Put('notification-preferences') @RequirePermission(Permissions.NOTIFICATIONS_READ) setPrefs(@ZBody(prefsDto) b: z.infer<typeof prefsDto>) { return this.svc.updatePreferences(b.items); }
}
