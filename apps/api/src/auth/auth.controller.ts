import { Body, Controller, Get, HttpCode, HttpException, Param, Patch, Post, Put, Query, Res } from '@nestjs/common';
...
export class AuthController {
  ...
  @Public() @Get('auth/config') @ApiOperation({ summary: 'Metodi di accesso disponibili per un tenant (password, SSO, login di sviluppo)' })
  config(@Query('tenant') tenant?: string) { return this.auth.publicConfig(tenant ?? ''); }

  @Public() @Post('auth/login') @HttpCode(200) @ApiOperation({ summary: 'Login con email e password; blocco temporaneo dopo 5 tentativi' })
  login(@Body(new ZodValidationPipe(loginDto)) b: z.infer<typeof loginDto>) { return this.auth.passwordLogin(b.tenantSlug, b.email, b.password); }
  ...
  forgot(@Body(new ZodValidationPipe(forgotDto)) b: z.infer<typeof forgotDto>) { return this.auth.forgotPassword(b.tenantSlug, b.email); }

  @Public() @Post('auth/reset-password') @HttpCode(200)
  reset(@Body(new ZodValidationPipe(resetDto)) b: z.infer<typeof resetDto>) { return this.auth.resetPassword(b.token, b.password); }
  ...
  accept(@Param('token') token: string, @Body(new ZodValidationPipe(acceptDto)) b: z.infer<typeof acceptDto>) { return this.auth.acceptInvite(token, b.password); }
  ...
  async oidcStart(@Query(new ZodValidationPipe(startQuery)) q: z.infer<typeof startQuery>, @Res() reply: FastifyReply) {
    const { url } = await this.auth.oidcStart(q.tenant, q.redirectTo);
    return reply.redirect(url, 302);
  }

  @Public() @Get('auth/oidc/callback')
  async oidcCallback(@Query(new ZodValidationPipe(callbackQuery)) q: z.infer<typeof callbackQuery>, @Res() reply: FastifyReply) {
  ...
  exchange(@Body(new ZodValidationPipe(exchangeDto)) b: z.infer<typeof exchangeDto>) { return this.auth.exchange(b.code); }
  ...
  async changePassword(@Body(new ZodValidationPipe(changeDto)) b: z.infer<typeof changeDto>) {
    const p = principal();
  ...
  async putSso(@Body(new ZodValidationPipe(ssoDto)) b: z.infer<typeof ssoDto>) {
    const p = principal();
