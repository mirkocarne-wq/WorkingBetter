...
export function WelfareRequestForm({ planId, categories, catalog, available, itemId }: { planId: string; categories: WelfareCategory[]; catalog: WelfareCatalogItem[]; available: number; itemId?: string }) {
  ...
  return (
    <form action={action} style={{ display: 'grid', gap: 8 }}>
      ...
      {!item && <label>Categoria<select name="categoryKey" value={categoryKey} onChange={(e) => setCategoryKey(e.target.value)} className="input">{categories.map((c) => <option key={c.key} value={c.key}>{c.name}</option>)}</select></label>}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        <label>Importo (€)<input name="amount" type="number" step="0.01" min={0.01} max={available} required value={amount || ''} onChange={(e) => setAmount(Number(e.target.value))} readOnly={!!item?.price} className="input" /></label>
        <label>Beneficiario<select name="beneficiary" defaultValue={cat?.beneficiaries.includes('self') ? 'self' : 'family'} className="input">{cat?.beneficiaries.includes('self') && <option value="self">Me stesso/a</option>}{cat?.beneficiaries.includes('family') && <option value="family">Familiare</option>}</select></label>
      </div>
      ...
      <input name="beneficiaryName" placeholder="Nome del familiare (se applicabile)" className="input" />
      {kind === 'reimbursement' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <label>Data della spesa<input name="expenseDate" type="date" className="input" /></label>
          <label>Giustificativo (nome file)<input name="attachmentName" placeholder="ricevuta.pdf" required className="input" /></label>
        </div>
      )}
      <textarea name="note" rows={2} placeholder="Note per chi verifica (facoltative)" className="input" style={{ resize: 'vertical' }} />
