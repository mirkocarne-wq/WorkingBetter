...
export interface OnboardingJourney extends OnboardingJourneySummary { viewer: 'self' | 'manager' | 'hr' | 'participant'; phases: OnboardingPhase[]; hr: PersonLite | null; it: PersonLite | null; appInstanceId: string | null; external: { email: string | null; expiresAt: string | null; active: boolean } | null; can: { edit: boolean; addTask: boolean; assignBuddy: boolean; sendExternalLink: boolean }; tasks: OnboardingTask[]; surveys: { key: string; title: string; score: number | null; low: boolean; answers: Record<string, number> | null; comment: string | null; submittedAt: string }[]; surveyDefs: Record<string, OnboardingSurveyDef> }
export interface OnboardingExternalTask extends OnboardingTask { form: { responseId: string; status: string; schema: FormSchemaDef; answers: Record<string, unknown> } | null }
export interface OnboardingExternal { organization: string; person: PersonLite | null; templateName: string; anchorDate: string; manager: PersonLite | null; buddy: PersonLite | null; phases: OnboardingPhase[]; tasks: OnboardingExternalTask[]; progress: { total: number; done: number } }
...
export interface AppInstance { id: string; appId: string | null; appKey: string; appVersion: number; name: string; icon: string | null; naming: AppDefinition['naming']; status: 'running' | 'completed' | 'cancelled'; outcome: string | null; title: string | null; currentStages: string[]; managedByModule: boolean; moduleLink: string | null; subject: PersonLite | null; launcher: PersonLite | null; startedAt: string; completedAt: string | null; cancelledAt: string | null; viewer: 'hr' | 'subject' | 'launcher' | 'actor' | 'manager'; progress: { total: number; done: number; active: number; percent: number }; can: { cancel: boolean; manage: boolean }; stages: AppInstanceStage[]; events: { at: string; type: string; stageKey: string | null; actor: string; data: Record<string, unknown> }[] }
