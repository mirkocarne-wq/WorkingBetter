...
export async function GET(req: Request) {
  ...
  const path = report === 'audit' ? '/audit/export' : report === 'saved' ? `/analytics/reports/${encodeURIComponent(reportId)}/run` : report === 'alerts' ? '/analytics/alerts' : report === 'process' ? `/analytics/process/${encodeURIComponent(url.searchParams.get('cycleId') ?? '')}` : report === 'welfare-payroll' ? `/welfare/payroll/batches/${encodeURIComponent(batchId)}/csv` : report === 'f360-aggregate' ? `/f360/campaigns/${encodeURIComponent(campaignId)}/aggregate` : report === 'app-instances' ? '/apps/instances' : '/analytics/query';
