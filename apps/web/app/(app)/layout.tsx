...
import { ApiError, apiFetch, MANUAL_URL, type Cycle, type Me, type Tenant } from '@/lib/api';
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  try {
    ...
    [unread, tenant, cycle] = await Promise.all([
    ...
    <AppShell
      ...
      sidebar={
        <>
          ...
          <SearchButton />
          <NavLinks permissions={me.permissions} unread={unread} />
          <div className="me">
      ...
      topbar={
        <>
          <PageCrumb />
          <span className="sp" />
          {cycle && <span className="chip" title={`Periodo attivo: ${cycle.startDate} – ${cycle.endDate}`}><Icon name="clock" size={14} />{cycle.name} · {shortDate(cycle.startDate)} – {shortDate(cycle.endDate)}</span>}
          ...
          <CommandPalette permissions={me.permissions} canSearchPeople={me.permissions.includes('people:read')} />
        </>
      }
    >
      {children}
