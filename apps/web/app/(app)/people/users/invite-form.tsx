...
export function InviteForm({ people, managers, units }: { people: { id: string; name: string; email: string | null }[]; managers: { id: string; name: string }[]; units: { id: string; name: string }[] }) {
  ...
  return (
    <form action={action} style={{ display: 'grid', gap: 8 }}>
      ...
      {mode === 'existing' ? (
        <>
          <label>Persona<select name="personId" value={personId} onChange={(e) => setPersonId(e.target.value)} style={input}>{people.map((p) => <option key={p.id} value={p.id}>{p.name}{p.email ? ` · ${p.email}` : ''}</option>)}</select></label>
          <label>Email di accesso<input name="email" type="email" required key={personId} defaultValue={selected?.email ?? ''} style={input} /></label>
        </>
      ) : (
        <>
          <div style={{ display: 'flex', gap: 8 }}><label style={{ flex: 1 }}>Nome<input name="firstName" required style={input} /></label><label style={{ flex: 1 }}>Cognome<input name="lastName" required style={input} /></label></div>
          <label>Email<input name="email" type="email" required style={input} /></label>
          <label>Ruolo aziendale<input name="jobTitle" placeholder="es. Account manager" style={input} /></label>
          <div style={{ display: 'flex', gap: 8 }}>
            <label style={{ flex: 1 }}>Manager<select name="managerId" defaultValue="" style={input}><option value="">—</option>{managers.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</select></label>
            <label style={{ flex: 1 }}>Unità<select name="orgUnitId" defaultValue="" style={input}><option value="">—</option>{units.map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}</select></label>
          </div>
        </>
      )}
      <div>
