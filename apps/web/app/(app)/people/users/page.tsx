...
import { apiFetch, fmtDate, roleLabel, type Me, type Person, type UserAdmin } from '@/lib/api';
...
const ROLES = ['employee', 'manager', 'hrbp', 'hr_admin', 'analyst', 'observer', 'tenant_admin'];

export default async function UsersPage() {
  const me = await apiFetch<Me>('/me');
  if (!me.permissions.includes('roles:manage')) redirect('/people');
  const [users, people, units] = await Promise.all([
  ...
  return (
    <>
      <div className="ph">
        ...
        <div style={{ display: 'flex', gap: 8 }}><Link href="/people" className="btn">Persone</Link>{me.permissions.includes('tenant:settings') && <Link href="/settings" className="btn">Impostazioni e SSO</Link>}</div>
      </div>
      <div className="grid" style={{ gridTemplateColumns: 'minmax(0, 1.6fr) minmax(0, 1fr)', alignItems: 'start' }}>
                  ...
                  <tr key={u.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, alignItems: 'center' }}>
                        {u.roles.map((r) => <form key={r.id} action={revokeRole.bind(null, r.id)} style={{ display: 'inline' }}><button className="pill b" title="Rimuovi ruolo" style={{ border: 0, cursor: 'pointer', font: 'inherit' }}>{roleLabel[r.role] ?? r.role} ×</button></form>)}
                        <form action={assignRole.bind(null, u.id)} style={{ display: 'inline-flex', gap: 4 }}>
                          <select name="role" className="btn sm" style={{ padding: '3px 6px' }} defaultValue="">{[['', '+ ruolo'], ...ROLES.filter((r) => !u.roles.some((x) => x.role === r)).map((r) => [r, roleLabel[r]])].map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select>
        ...
        <div className="card">
          ...
          <InviteForm people={withoutUser.map((p) => ({ id: p.id, name: `${p.firstName} ${p.lastName}`, email: p.email }))} managers={people.map((p) => ({ id: p.id, name: `${p.firstName} ${p.lastName}` }))} units={units} />
