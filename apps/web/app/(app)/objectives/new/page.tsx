      ...
      <div className="grid" style={{ gridTemplateColumns: canCycles ? '1.6fr 1fr' : '1fr', alignItems: 'start' }}>
        {open.length === 0 ? <div className="card empty">Nessun periodo aperto: {canCycles ? 'creane uno qui a destra.' : 'chiedi all’HR di aprire un periodo.'}</div> : (
          <form action={createObjective} className="card" style={{ display: 'grid', gap: 12 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
              <label>Periodo<select name="cycleId" defaultValue={cycleId} style={input}>{open.map((c) => <option key={c.id} value={c.id}>{c.name} · {c.startDate} → {c.endDate}</option>)}</select></label>
              <label>Livello<select name="level" defaultValue="individual" style={input}>{levels.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></label>
              <label>Visibilità<select name="visibility" defaultValue="public" style={input}><option value="public">Pubblico</option><option value="team">Solo team</option><option value="private">Privato (io e il mio manager)</option></select></label>
            </div>
            <label>Titolo<input name="title" required maxLength={200} placeholder="Ridurre il tempo di risposta ai ticket P1 sotto le 4 ore" style={input} /></label>
            <label>Descrizione<textarea name="description" rows={2} placeholder="Perché conta e cosa cambia se lo raggiungiamo" style={{ ...input, resize: 'vertical' }} /></label>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
              <label>Owner<select name="ownerPersonId" defaultValue={myId ?? ''} style={input}>{owners.map((p) => <option key={p.id} value={p.id}>{p.firstName} {p.lastName}{p.id === myId ? ' (io)' : ''}</option>)}</select></label>
              {canTeam && <label>Unità (per obiettivi di unità/team)<select name="ownerOrgUnitId" defaultValue="" style={input}><option value="">—</option>{units.map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}</select></label>}
              <label>Scadenza<input name="dueDate" type="date" style={input} /></label>
            </div>
            <label>Allineato a<select name="parentId" defaultValue="" style={input}><option value="">Nessun obiettivo padre</option>{parents.filter((o) => o.status === 'active' || o.status === 'draft').map((o) => <option key={o.id} value={o.id}>[{levelLabel[o.level]}] {o.title}</option>)}</select><div className="sup">Il progresso dei key result risale lungo l&apos;albero di allineamento.</div></label>
        ...
        {canCycles && (
          <form action={createOkrCycle} className="card" style={{ display: 'grid', gap: 8 }}>
            ...
            <label>Nome<input name="name" required placeholder="Q4 2026" style={input} /></label>
            <div style={{ display: 'flex', gap: 8 }}><label style={{ flex: 1 }}>Dal<input name="startDate" type="date" required style={input} /></label><label style={{ flex: 1 }}>Al<input name="endDate" type="date" required style={input} /></label></div>
            <label>Cadenza check-in (giorni)<input name="checkInCadenceDays" type="number" min={1} max={90} defaultValue={7} style={input} /></label>
