...
import { publishFormDefinition, startFormResponse } from '@/lib/actions';
          ...
          {(canManage ? forms : published).map((f) => (
            <div key={f.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--grid)' }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                {canManage && f.status === 'draft' && <form action={publishFormDefinition.bind(null, f.id)}><button className="btn sm p">Pubblica</button></form>}
              </div>
            </div>
          ))}
