...
export function SsoForm({ sso }: { sso: SsoConfig }) {
  ...
  return (
    <form action={action} style={{ display: 'grid', gap: 10 }}>
      ...
      <label>Issuer (URL del provider)<input name="issuer" defaultValue={sso.issuer} placeholder="https://login.microsoftonline.com/<tenant>/v2.0 · http://localhost:8080/realms/workingbetter" style={input} /></label>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <label>Client ID<input name="clientId" defaultValue={sso.clientId} style={input} /></label>
        <label>Client secret {sso.hasClientSecret && <span className="sup">(già salvato, cifrato)</span>}<input name="clientSecret" type="password" autoComplete="off" placeholder={sso.hasClientSecret ? 'lascia vuoto per non cambiarlo' : 'facoltativo con PKCE'} style={input} /></label>
      </div>
      ...
      <label>Domini email ammessi <span className="sup">(separati da virgola; vuoto = tutti)</span><input name="allowedDomains" defaultValue={sso.allowedDomains.join(', ')} placeholder="acme.it, acme.com" style={input} /></label>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        ...
        <label>Ruolo di default<select name="defaultRole" defaultValue={sso.defaultRole} style={input}><option value="employee">Collaboratore</option><option value="manager">Manager</option><option value="observer">Osservatore</option></select></label>
      </div>
