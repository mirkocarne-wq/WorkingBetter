                ...
                <tr key={r.id}>
                  ...
                  <td>
                    <form style={{ display: 'grid', gap: 4, minWidth: 220 }}>
                      <input name="note" placeholder="Nota per il dipendente" className="input" style={{ padding: '4px 8px', fontSize: 12 }} />
      ...
      {tab === 'plans' && (
        <div className="grid" style={{ gridTemplateColumns: 'minmax(0, 1.4fr) minmax(0, 1fr)', alignItems: 'start' }}>
              ...
              <div className="card">
                ...
                {selectedPlan.status !== 'closed' && (
                  <form action={addWelfareSource.bind(null, selectedPlan.id)} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 0.8fr 1fr 1fr auto', gap: 6, alignItems: 'end', marginTop: 10 }}>
                    <label>Nome<input name="name" required placeholder="Budget on top" className="input" /></label>
                    <label>Tipo<select name="kind" className="input"><option value="on_top">On top</option><option value="ccnl">Da CCNL</option><option value="manual">Ricarica</option></select></label>
                    <label>€/persona<input name="amountPerPerson" type="number" step="0.01" min={0} required className="input" /></label>
                    <label>Accredito<input name="creditAt" type="date" required defaultValue={selectedPlan.periodStart} className="input" /></label>
                    <label>Scadenza<input name="expiresAt" type="date" defaultValue={selectedPlan.periodEnd} className="input" /></label>
                ...
                <details style={{ marginTop: 12 }}>
                  ...
                  <form action={adjustWelfare} style={{ display: 'grid', gridTemplateColumns: '1.5fr 0.8fr 2fr auto', gap: 6, alignItems: 'end', marginTop: 8 }}>
                    ...
                    <label>Persona<select name="personId" className="input">{people.map((p) => <option key={p.id} value={p.id}>{p.firstName} {p.lastName}</option>)}</select></label>
                    <label>Importo (± €)<input name="amount" type="number" step="0.01" required className="input" /></label>
                    <label>Motivazione<input name="note" required minLength={3} className="input" /></label>
          ...
          <form action={createWelfarePlan} className="card" style={{ display: 'grid', gap: 8 }}>
            <h3>Nuovo piano</h3>
            <label>Nome<input name="name" required placeholder={`Welfare ${year + 1}`} className="input" /></label>
            <div style={{ display: 'grid', gridTemplateColumns: '0.7fr 1fr 1fr', gap: 8 }}>
              <label>Anno<input name="year" type="number" defaultValue={year} className="input" /></label>
              <label>Dal<input name="periodStart" type="date" required defaultValue={`${year}-01-01`} className="input" /></label>
              <label>Al<input name="periodEnd" type="date" required defaultValue={`${year}-12-31`} className="input" /></label>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <label>Riporto a fine piano<select name="rolloverRule" className="input"><option value="none">Nessuno (il residuo scade)</option><option value="total">Totale</option><option value="partial">Parziale (%)</option></select></label>
              <label>% riportata<input name="rolloverPercent" type="number" min={0} max={100} defaultValue={0} className="input" /></label>
            </div>
            <label>Regolamento (testo)<textarea name="regulation" rows={3} className="input" style={{ resize: 'vertical' }} placeholder="Regole del piano, presa visione richiesta per la conversione del premio" /></label>
            <details><summary className="sup" style={{ cursor: 'pointer' }}>Conversione del premio di risultato</summary>
              <div style={{ display: 'grid', gap: 8, marginTop: 8 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}><label>Premio lordo (€)<input name="premiumAmount" type="number" step="0.01" className="input" /></label><label>Finestra dal<input name="premiumFrom" type="date" className="input" /></label><label>al<input name="premiumTo" type="date" className="input" /></label></div>
        ...
        <div className="grid" style={{ gridTemplateColumns: 'minmax(0, 1.4fr) minmax(0, 1fr)', alignItems: 'start' }}>
          ...
          <div style={{ display: 'grid', gap: 16 }}>
            <form action={createCatalogItem} className="card" style={{ display: 'grid', gap: 8 }}>
              <h3>Nuova voce</h3>
              <label>Nome<input name="name" required className="input" /></label>
              <label>Descrizione<input name="description" className="input" /></label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <label>Categoria<select name="categoryKey" className="input">{categories.map((c) => <option key={c.key} value={c.key}>{c.name}</option>)}</select></label>
                <label>Tipo<select name="kind" className="input"><option value="reimbursement">Rimborso</option><option value="voucher">Voucher</option><option value="service">Servizio</option></select></label>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}><label>Prezzo fisso (€)<input name="price" type="number" step="0.01" className="input" /></label><label>Importo massimo (€)<input name="maxAmount" type="number" step="0.01" className="input" /></label></div>
              <label>Istruzioni<input name="instructions" className="input" /></label>
            ...
            <form action={createInitiative} className="card" style={{ display: 'grid', gap: 8 }}>
              ...
              <label>Nome<input name="name" required className="input" /></label>
              <label>Descrizione<input name="description" className="input" /></label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}><label>Tipo<select name="kind" className="input"><option value="convention">Convenzione</option><option value="program">Programma</option><option value="event">Evento</option></select></label><label>Posti (vuoto = illimitati)<input name="capacity" type="number" min={1} className="input" /></label></div>
              <label>Come aderire<input name="howTo" className="input" /></label>
        ...
        <div className="grid" style={{ gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', alignItems: 'start' }}>
          <div className="card">
            ...
            <form action={createPayrollBatch} style={{ display: 'flex', gap: 8, alignItems: 'end', marginTop: 12 }}>
              <label style={{ flex: 1 }}>Periodo cedolino<input name="period" defaultValue={new Date().toISOString().slice(0, 7)} pattern="\d{4}-\d{2}" className="input" /></label>
